  ...
  return (
    <div className="flex justify-center items-center">
      <div className="w-2/3 md:w-1/2 lg:w-1/3 rounded-[2rem] border-[0.031rem] border-white bg-foreground/20 backdrop-blur-lg shadow-[0_0px_18px_#ffffff] py-7 px-6 lg:py-11 lg:px-10 flex flex-col gap-4">
        {title && <h1 className={cn("font-cinzel font-black text-white text-xl leading-7 md:text-2xl md:leading-8 lg:text-3xl lg:leading-9", titleClassName)}>{title}</h1>}
        {icon && (
        ...
        {description && (
          <p className={cn("font-openSans text-sm leading-6 lg:text-base text-white", descriptionClassName)}>
            {description}
          </p>
        )}
        {button && button.length > 0 && (
        <div className={cn(`flex ${button.length === 1 ? "justify-center" : "gap-2"}`, buttonClassName)}>
